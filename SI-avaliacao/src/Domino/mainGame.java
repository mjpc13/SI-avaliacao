...
public class mainGame {
	...
	public static player newGame(boolean extra) throws InterruptedException { // funcao que inicia um jogo, cria os objectos necessário para se jogar
	...
	public static player game(ArrayList<player> listofPlayers, int set, board board) throws InterruptedException{
		...
		ArrayList<player> listofDealers = dealPieces(setDealer(listofPlayers)); // ajusta a lista de dealers e destribui as peças pelos jogadores.
		...
		for(player pl : listofPlayers) {						//ciclo para adicionar os pontos do set aos pontos do jogo de cada jogador
			...
			if(pl.gamePoints>=50) { //se algum jogador tiver mais de 50pt, o jogo acaba, então a variavel endgame para a true
				endgame=true;
		...
		String answer = print.printExitOption(); // opção para sair do jogo. O jogador só consegue desistir no final de cada set
		...
		if(answer.equalsIgnoreCase("Y")) {
			...
			if(!endgame) { // se o jogo ainda não acabou
				...
				game(listofPlayers, set +1, board); // chama-se outra vez a função game (isto entra num ciclo de chamar esta função até engame==true)
				
			}
			
			else {// se o jogo acabou
				
				for(int i=0;i<4;i++) { //ciclo que vê qual dos jogadores tem MENOS pontos (este será o vencedor do jogo) 
					
					
					if(listofPlayers.get(i).gamePoints<=game_points) {  //se o jogador actual tiver menos pontos que uma variavel (inicialmente de valor 100, e dps toma os pontos do jogador se este tiver menos pontos que a mesma)
						...
						game_points=listofPlayers.get(i).gamePoints; // atribui o numero de pontos a variavel de comparação e depois assume que este jogador ganhou (se não tiver ganho, a linha 242 do código acaba por meter false)
	...
	public static ArrayList<player> set(ArrayList<player> listofPlayers ,ArrayList<player> listofDealers, int round, board board) throws InterruptedException {
		...
		if(round==0) { // se for a primeira ronda é preciso definir a ordem de jogada
			...
			print.dealer(listofDealers); //faz print da lista de dealers (nota que esta só é gerada no inicio do jogo e ordem mantem/se igual)
		...
		for(player pl : listofPlayers) { // lista que percorre todos os jogadores e estes vão jogando
			...
			if(board.dominoesInBoard.equals(board_tie)) { //se o novo tabuleiro for igual ao board_tie, significa que o pl passou a sua vez, e o counter incrementa por 1;
			...
			if(pl.hand.isEmpty()) { //se o jogador depois de jogar ficar com a mão vazia significa que ganhou e sendo assim o set acabou, endset=true

					
					endset=true;
					
					for(player pl1 : listofPlayers) { // iguala o numero de pontos na mão de cada jogador ao numero final do set do jogador
				...
				break;// faz break para garantir que mais nenhum jogador jogar depois de alguém ganhar (break do ciclo da linha 291)
		...
		if(counter==4) { // se counter chegar a 4, significa que os jogadores passaram todos, logo o jogo empata e acaba
			endset=true;
	...
	public static ArrayList<player> setDealer(ArrayList<player> listofPlayers) { //função que define os dealers
		...
		if( listofPlayers.get(0).dealer ) { 		// se o primeiro elemento for um dealer (significa que esta função já foi chamada), assume como o 2 elemento o dealer, remove o primeiro e adidiona-o no fim da lista
		...
		else {										// se o primeiro elemento não for dealer, significa que este ainda não foi atribuido
			int rnd =rand.nextInt(4);
			listofPlayers.get(rnd).dealer=true;		// escolhe um dealer aleatóriamente
			
			while(!listofPlayers.get(0).dealer) {	// enquanto o 1 elemento não for dealer, iremos remover o 1 elemento e colocar no fim da lista
	...
	public static ArrayList<player> setPlayers(ArrayList<player> listofPlayers){ //define a ordem dos jogadores
		...
		while(!listofPlayers.get(0).winner) { // como o vencedor tem de ser o primeiro a jogar, retiramos o primeiro elemento da lista e passamos para ultimo até que o elemento 0 seja o vencedor
	...
	//Funções para o Extra... Funcionam como as outras, mas têm pequenas diferenças no que toca a pontuação e prints, NÃO SÃO ELEMENTOS DE AVALIAÇÃO PODEM SER IGNORADAS
	...
	public static ArrayList<player> setExtra(ArrayList<player> listofPlayers ,ArrayList<player> listofDealers, int round, board board) throws InterruptedException {

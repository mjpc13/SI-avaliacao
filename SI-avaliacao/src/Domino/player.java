...
public class player {
	...
	boolean human; // boolean para ver se o jogador é humano ou não (se for humano fica com valor
					// de TRUE)
	...
	public board play(board board) throws InterruptedException {
		...
		if (human) { // caso em que o jogador é humano
			...
			for (int i = 0; i < hand.size(); i++) {
				...
				if (board.dominoesInBoard.size() != 0) { // garantir que o tabuleiro não está vazio, se estiver todas as
															// peças são admissiveis
					if (hand.get(i).num1.equals(board.bottomEdge()) || hand.get(i).num2.equals(board.upperEdge())
							|| hand.get(i).num2.equals(board.bottomEdge())
							|| hand.get(i).num1.equals(board.upperEdge())) {

						validPlays[i] = String.valueOf(i + 1); // se a peça puder ser jogada, adiciona o número
																// correspondente a lista de opções válidas
			...
			if (board.dominoesInBoard.isEmpty()) {
				System.out.println("\n Choose the piece you want to play (press [P] to pass)");// print para quando está
																								// vazio
			} else {// print que avisa o jogador qual os numeros jogáveis
				System.out.println("\n Choose the piece you want to play (press [P] to pass)" + ": ( ↑"
						+ board.upperEdge() + ", ↓" + board.bottomEdge() + " )");
			}
			...
			for (int i = 0; i < hand.size(); i++) { // igual ao jogador, para verificar que opções são válidas (podia
													// estar numa função a parte para ser lida mais facilmente)
			...
			for (String i : validPlays) { // percorre as opções válidas (e irá escolher a que a soma dos pontos seja a
											// maior [em caso de empate é a última])
				...
				if (i == null) {
					...
					if (cp_domino == null) {
						return board; // se cp_domino não tiver valor significa que não há peças válidas e o cp é
										// obrigado a passar
					}
					...
					if (cp_domino == null) { // se o cp_domino ainda não tiver valor, e a opção for nula, vai ficar com
												// esta peça
					...
					else if (cp_domino.points < hand.get(Integer.parseInt(i) - 1).points) { // sempre que a peça tenha
																							// uma soma superior ao
																							// cp_domino, o valor
																							// do cp_domino é atualizado
																							// para a nova peça
						cp_domino = hand.get(Integer.parseInt(i) - 1);
		...
		if (human) {
			...
			for (String i : validPlays) {
				...
				if (i == null) {
					...
					// da maneira como construi o código é complicado eu dizer ao jogador para
					// tentar fazer 0/5/10 pontos de preferencia, vou deixar assim...
